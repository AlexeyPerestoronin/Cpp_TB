...
namespace TB_NS::Error_NS {
    ...
    Exceptions::Exceptions()
        ...
        m_allExceptions.emplace_back(const_cast<Exception*>(m_rootException));
    }
    ...
    Exception& Exceptions::RegistNewException(Str::CR i_id, Str::CR i_key, Exception::Values::CR i_values) {
        ...
        exceptions.push_back(std::make_shared<Exception>(this, i_id, i_key, i_values));
    ...
    void Exceptions::LoadSettings(const char* i_settigsFilePath, size_t i_size) {
        using namespace nlohmann;

        std::string_view filePath{ i_settigsFilePath, i_size };

        json errors;
        if (auto settingsFile = RAI<std::ifstream>(filePath); settingsFile.is_open())
            settingsFile >> errors;
        else
            throw *m_rootException;
    ...
    Exception::CR Exceptions::operator[](Str::CR i_IdOrKey) const noexcept {
        ...
        if (const auto exception = std::ranges::find_if(exceptions, std::move(comparator)); exception != exceptions.end())
            return *(*exception);
        return const_cast<Exceptions*>(this)->RegistUnknowException(i_IdOrKey, i_IdOrKey);
    }

    Exceptions CreateExceptionsIns() {
        return Exceptions();
    }
} // namespace TB_NS::Error_NS
