...
namespace TB_NS::Trader_NS::Interaction_NS::Commands_NS {
    ...
    OrdersBook::Responce OrdersBook::getR(TradePair i_pair) {
        Json responce =  m_exchange->interact(code(), RequestedParameters{ std::move(i_pair), std::move(Limit{100}) });
        Str str = responce.dump(4);
        return Responce{};
    }

    OrdersBook::Responce OrdersBook::request(TradePair i_pair, Limit i_limit) const {
        /*Json responce = */m_exchange->interact(code(), RequestedParameters{ std::move(i_pair), std::move(i_limit) });
        return Responce();
